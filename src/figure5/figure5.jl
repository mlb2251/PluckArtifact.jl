...
function warmstart_config(x::GroupConfig)
    GroupConfig(
...
function make_all_datasets()
    for task in all_tasks
        println("Making dataset for $task")
        task_dist = grammar_of_task(task)
        make_fuzzing_dataset(; task_dist)
    end
end
...
function make_fuzzing_dataset(; N=100, M=4, E=4, task_dist=map_unit_grammar())
    ...
    env_names = [:xs]
    start = parsed_expr(task_dist.grammar, task_dist.grammar.start_expr_of_type[output_type], output_type, input_types, env_names)
    ...
    while length(dataset.tasks) < N
        ...
        # Sample expr
        size_dist = Uniform(0, 10)
        # size_dist = Uniform(3, 3)
        size = random(size_dist)
        expr = random(fixed_size_dist, task_dist.grammar, size, from_lhs_detached(start))

        q_expr = logprob(fixed_size_dist, task_dist.grammar, size, expr) + logprob(size_dist, size)
        ...
        for i in 1:M+E
            ...
            # @show input
            # @show task_dist.input_dist
            p_input = get_true_result(Pluck.compile("(=? $(input) $(task_dist.input_dist))"))
            ...
            for j in 1:20
                try
                    output = sample_output(string(expr); env=EnvCons(env_names[1], input, EnvNil()), max_depth=1000)
                catch e
                    e isa StackOverflowError && continue
                    rethrow(e)
                end
            ...
            isnothing(output) && break
            # @show expr
            input_val, concrete = from_value(input)
            output_val, concrete = from_value(output)
            !task_dist.valid_io(input_val, output_val) && break
...
function make_tc(mode::Symbol; temperature=1.0, train_only=false, kwargs_of_task)
    ...
    if mode == :bdd
        return task -> TaskConstrain(task, Pluck.LazyKCConfig(; kwargs_of_task(task)...); temperature, train_only)
    elseif mode == :lazy
...
function get_historical_data()
    ...
    open(joinpath(target_dir, "revamp_may12.json")) do f # historical_nov14.json
        data = JSON.parse(f)
        for group in data["groups"]
            if haskey(group, "gen_file") && haskey(group, "eval_file")
                ...
                gen_file = group["gen_file"]
                eval_file = group["eval_file"]
